...
class PytorchEngineConfig:
    ...
    def __post_init__(self):
        ...
        assert self.kernel_block_size >= 16 and (self.kernel_block_size & (self.kernel_block_size - 1)) == 0, \
        f'kernel_block_size must be >= 16 and a power of 2, but got {self.kernel_block_size}'
        assert self.block_size >= self.kernel_block_size and self.block_size % self.kernel_block_size == 0, \
            f'block_size must be >= kernel_block_size and an integer multiple of kernel_block_size, but got block_size {self.block_size} and kernel_block_size {self.kernel_block_size}'
